...
class snatch extends SystemAction {
	public function sc(){
		...
		$mygm = $db->GetList("select shopqishu,shopid from `@#_member_go_record` where uid='$info[uid]' ");
			...
			foreach($mygm as $v){
				$zhsz[] = $db->GetList("select a.q_end_time,a.title,a.thumb,a.money,b.mobile from `@#_shoplist` as a,`@#_member` as b where a.q_uid=b.uid and a.qishu='$v[shopqishu]' and a.id='$v[shopid]' and  a.q_end_time is not null and a.q_user is not null ");				
			}
			$data=array_filter($zhsz,create_function('$v','return !empty($v);'));
			foreach($data as $k=>$v){
	        	foreach($v as $i=>$j){
