...
class mobile extends base {
	...
	public function init(){
		...
		//十元区
		$shiyuan_shop=$this->db->GetList("select * from `@#_shoplist` where `shiyuan` = '1' and `q_end_time` is null ORDER BY `id` DESC LIMIT 2");
		//百元区
		$baiyuan_shop=$this->db->GetList("select * from `@#_shoplist` where `baiyuan` = '1' and `q_end_time` is null ORDER BY `id` DESC LIMIT 2");
		$max_renqi_qishu = 1;
	...
	public function glistajax(){
		...
		if($select == 20){
			$select_w = "and `shiyuan` = '1'";
		}
		if($select == 30){
			$select_w = 'order by `shenyurenshu` ASC';
		}
		if($select == 40){
			$select_w = "and `baiyuan` = '1'";
		}
		if($select == 50){
