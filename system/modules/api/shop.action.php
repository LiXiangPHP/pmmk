...
class shop extends SystemAction {
	...
	public function json_shopshare() {
		...
		foreach($Sdata as $v) {
			$aa = $this->db->GetOne("select title, q_user_code code from `@#_shoplist` where `id`='$v[shopid]' ");			
	...
	public function json_indiana() {
		...
		if($item) {
			...
			$page=System::load_sys_class('page');
			$page->config($total,$num,$pagenum,"0");
			$Idata = $this->db->GetPage("SELECT id,username,uphoto,time, gonumber part,ip FROM `@#_member_go_record`  where  shopid = '$id' order by time desc",array("num"=>$num,"page"=>$pagenum,"type"=>1,"cache"=>0));
			foreach($Idata as $v) {
