...
class index1 extends SystemAction {
	...
	public function kj(){
		...
		$data = $db->GetPage("select qishu,id,canyurenshu,shenyurenshu,title,money,thumb from `@#_shoplist` where `q_uid` is null and shengyurenshu>0 order by shenyurenshu asc ",array("num"=>$num,"page"=>$pagenum,"type"=>1,"cache"=>0));
		foreach($data as $k=>$v) {
