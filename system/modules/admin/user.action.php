...
class user extends admin {
	...
	public function reg(){
		...
		if(isset($_POST['submit-1'])){
			...
			$shop = $_POST['shop'];
			$user = $_POST['user'];
			$yemian = $_POST['jiemian'];
			$yun = $_POST['yun'];
			$this->db->Query("INSERT INTO `@#_admin` (`mid`, `username`, `userpass`, `useremail`, `addtime`, `logintime`, `loginip`, `xitong`, `neirong`, `shop`, `user`, `yemian`, `yun`) VALUES ('$pmid', '$username', '$password', '$_POST[email]','$addtime','0','$ip','$xitong','$neirong','$shop','$user','$yemian','$yun')");
	...
	public function edit(){
		...
		if(isset($_POST['submit-1'])){			
			...
			if(!$password1)
			{
				$ok = $this->db->Query("UPDATE `@#_admin` SET `xitong`='$xitong',`neirong`='$neirong',`shop`='$shop',`user`='$user',`yemian`='$yemian',`yun`='$yun' WHERE (`uid`='$uid')");
			}
			
			else
			{
				$ok = $this->db->Query("UPDATE `@#_admin` SET `userpass`='$password',`xitong`='$xitong',`neirong`='$neirong',`shop`='$shop',`user`='$user',`yemian`='$yemian',`yun`='$yun' WHERE (`uid`='$uid')");
			}
			if($ok){
				echo "<script>
				alert('修改成功');
