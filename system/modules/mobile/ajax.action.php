...
# 我的修改
	...
	{	
		...
		$member=$this->db->GetOne("SELECT * FROM `@#_member` WHERE `username` = '$username' LIMIT 1");
		if($member)
		{
			$item['code']=1;
			$item['msg']='昵称已存在';
	...
	function addAddress()
	{
		...
		foreach($_POST as $k=>$v){
			...
			$sheng=isset($_POST['province']) ? $_POST['province'] : "";
			$shi=isset($_POST['city']) ? $_POST['city'] : "";
			$xian=isset($_POST['county']) ? $_POST['county'] : "";
			$jiedao=isset($_POST['address']) ? $_POST['address'] : "";
			$youbian=isset($_POST['youbian']) ? $_POST['youbian'] : "";
			$shouhuoren=isset($_POST['shouhuoren']) ? $_POST['shouhuoren'] : "";
			$mobile=isset($_POST['shouji']) ? $_POST['shouji'] : "";
			$time=time();
			...
			if(!$member_dizhi){
				...
				$default="N";
					...
					$mysql_model->Query("INSERT INTO `@#_member_dizhi`(`uid`,`sheng`,`shi`,`xian`,`jiedao`,`youbian`,`shouhuoren`,`mobile`,`default`,`time`)VALUES
			('$uid','$sheng','$shi','$xian','$jiedao','$youbian','$shouhuoren','$mobile','$default','$time')");
					$item['code'] =0;
					echo json_encode($item);die;
	}
	//删除收货地址
